...
The architecture comprises four layers, each with a distinct responsibility. The layers are not optional substitutes for one another. Given the local-first commitment, a system that omits any one of them is not local-first at the seam; it has merely relocated the trust assumption elsewhere.
...
**Known Limit — KL-README-01:** Relay institutional exit is not an architectural property. In regulated-domain deployments (settlement rails, HIPAA BAA relationships, AML/KYC retention workflows), the relay's continued institutional participation is legally or contractually required regardless of the architecture's content-encryption approach. These are known boundary conditions of the relay-exit principle, not cases awaiting a cryptographic solution.
...
3. **A gate.** Every crossing attempt is checked against the grant's current capability state at act time. The gate fails closed: unconfirmed revocation blocks; silence blocks.
...
**[Pattern Commons #9 — The Substrate-Crossing Seam](https://github.com/jediwright/local-first-series/blob/main/pattern-commons/pattern-commons-09-governed-content-production-crossing.md)** governs the crossing that fires when a content object is submitted from a local-first, author-controlled substrate to an externally legible surface — a CMS, a public web endpoint, or an AT Protocol record — making TCF tier boundaries enforcement gates rather than editorial conventions. Specified at v0.3 after a third Counter-Pass converged with narrowing; three surface classes declared, one evidenced by PC#8; not yet prototyped. Its companion runtime, [tcf-runtime](https://github.com/jediwright/tcf-runtime), completed Phase 0 (write-time gate on fixtures, engine pinned) on 2026-09-11; PC#9 specifies reading that runtime's outputs, but nothing implements it yet.
...
Entries #1–#6 document earlier prototype-phase work in commerce, healthcare, social networking, and infrastructure. The series is not exhaustive and is not intended to be; the pattern can be instantiated in any domain where a governed crossing makes sense. Canonical entries represent the classes where the architecture has been stress-tested and formalized. Pattern-conformant implementations not submitted for canonical review are valid governed crossings; they simply have not entered the taxonomy index.
